...
#    ある変数「y」の変動を、別の変数「x」の変動により説明（予測）すること
#  ・回帰には、「説明変数」「目的変数」がある
#   ・「説明変数」(x)：何かの原因となっている変数
#   ・「目的変数」(y)：その原因を受けて発生した結果である変数
#  ・以下の「単回帰」は、最もシンプルな回帰
#     y = a * x + b
#  ・「重回帰」は、説明変数が、複数ある回帰

# ・使用するライブラリ：scikit-learn
#  ・世界中で広く使われている、Pythonの機械学習ライブラリ
#  ・Google Colaboratory（Colab）に標準でインストールされている
#  ・様々な機械学習アルゴリズムを含む
#   ・回帰
#   ・k近傍法
#   ・サポートベクターマシン
...
# ※※※．ここのコードは、元々のサンプル時点で、既に動かない（エラー発生）
